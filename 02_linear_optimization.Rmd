...
```{r}
library(linprog)
objective_values <- c(x=40, y=30)

margin_values <- c(12,16)

const_directions <- c("<=", "<=")

const_matrix <- rbind(c(1,1),
                      c(2,1))

test_lp<-solveLP(cvec = objective_values, bvec = margin_values, Amat = 
          const_matrix, maximum = TRUE, const.dir = const_directions)
```
```{r}
advert_data<- read.csv("advertising_data.csv",row.names = 1)
```
